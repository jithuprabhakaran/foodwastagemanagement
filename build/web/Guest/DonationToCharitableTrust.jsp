...
<%
    ...
    if(request.getParameter("UId")!=null){
        sel="select * from tbl_charitabletrust where chari_id="+request.getParameter("UId");
        rs=obj.select(sel);
        if(rs.next()){
            username=rs.getString("chari_name");
            address=rs.getString("chari_address");
            contact=rs.getString("chari_contact1");          
            uid=Integer.parseInt(rs.getString("chari_id"));
        }
    }
    if(request.getParameter("btnSubmit")!=null){
        ...
        str="insert into tbl_donation(guest_id,reguser_id,chari_id,don_date,don_time,don_type,don_worthamt,don_status) values("+
                guestid+",0,"+uid+",'"+date+"','"+time+"','"+type+"',"+amt+",'P')";
     
        if(obj.insert(str))
        {
            response.sendRedirect("DonationToCharitableTrust.jsp");
        }
...
<%@include  file="header.html" %>
 <link rel="stylesheet" type="text/css" href="../mystyle.css"/>
    ...
    <body onload="display_ct()">
        <form name="frmDonationToReguser" action="DonationToCharitableTrust.jsp" method="POST">
            <center>
            <table class="CSSTableGenerator2">
                    ...
                    <tr>
                     ...
                     <tr>
                        <td>Worth Amount</td>
                        <td><input type="text" name="txtAmount" value="" required="required" maxlength="5" /></td>
                    </tr>
                    <tr>
            ...
            <table class="CSSTableGenerator2">
                    <tr>
                        <td colspan="6">Charitable Trust Details</td>
                    ...
                    <%
                        sel="select * from tbl_charitabletrust where chari_status='A'";
                        rs=obj.select(sel);
                        while(rs.next())
                        {
                        %>
                    <tr>
                        ...
                        <td><a href="DonationToCharitableTrust.jsp?UId=<%=rs.getString("chari_id") %>">Select Charitable Trust</a></td>
                    </tr>
                    <%
                        }
                       %>
